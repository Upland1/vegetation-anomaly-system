...
Espera a que los agentes terminen su exploración (se detendrán).

Opción A (Recomendada): Presiona la tecla ESC en la ventana o cierra la ventana con la X.

Opción B (Forzada): Ve a la terminal y presiona Ctrl + C.
...
Exploración: El Manager divide el mapa y asigna zonas a cada Agente Físico. Inicializa 5 agentes físicos, cada uno en un thread distinto.

Sensado: Los agentes viajan a las celdas y generan datos simulados (humedad, maduración, plagas).

Comunicación: El Agente Físico envía datos al Manager mediante callbacks.

Decisión: El Manager evalúa reglas. Si detecta un jitomate maduro o una plaga crítica, crea una Instrucción.

Interrupción: El Manager coloca la instrucción en la "Cola de Tareas" del agente más cercano. El agente pausa su exploración, ejecuta la acción (cosechar/curar) y luego retoma su ruta.